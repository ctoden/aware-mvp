...
import { DiscViewModel } from '@src/viewModels/DiscViewModel';
...
const DiscScreen: React.FC = observer(() => {
  const router = useRouter();
  const { viewModel } = useViewModel(DiscViewModel);
  ...
  useEffect(() => {
    ...
    console.log("DISC Screen effect running with currentId:", assessmentViewModel.currentAssessment$.get());
    ...
    if (assessmentViewModel.currentAssessment$.get()) {
      ...
      // We're in create mode - find any existing DISC assessment
      existingAssessment = assessments.get().find(a => a.assessment_type === 'DISC');
      console.log("Create mode, found existing DISC:", existingAssessment);
    }
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View marginB-s8>
        <Text style={typography.h2} marginB-s2>DiSC®</Text>
        <Text style={[typography.bodyM, { color: colors.textSecondary }]}>
          Add your score out of 100 for each item.
        </Text>
      </View>
