...
@injectable()
export class DiscViewModel extends ViewModel {
  ...
  public readonly scores$ = observable<DiscScore[]>([
    { name: 'Dominance', score: '' },
    { name: 'Influence', score: '' },
    { name: 'Steadiness', score: '' },
  ...
  public readonly discFullTextResult$ = observable<string | null>(null);
  ...
  async saveAssessmentTextResults(text: string, fileName: string): Promise<Result<boolean, Error>> {
    try {
      ...
      this.discFullTextResult$.set(text);
  ...
  public async updateAssessment(assessmentId: string): Promise<Result<boolean, Error>> {
    if (!this.isSubmitEnabled()) {
      this.error$.set('Please enter all DISC scores before updating');
      return err(new Error('Please enter all DISC scores before updating'));
    }
    ...
    try {
      ...
      const scores = this.scores$.get().reduce((acc, score) => {
        acc[score.name.toLowerCase()] = parseInt(score.score);
        return acc;
      ...
      const result = await this._userAssessmentService.updateAssessment(assessmentId, {
        ...
        additional_data: {
          scores,
          assessmentResult: this.discFullTextResult$.get()
        }
      });
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to update DISC assessment';
      this.error$.set(errorMessage);
      return err(error instanceof Error ? error : new Error(errorMessage));
    } finally {
  ...
  public async submitAssessment(): Promise<Result<boolean, Error>> {
    if (!this.isSubmitEnabled()) {
      this.error$.set('Please enter all DISC scores before submitting');
      return err(new Error('Please enter all DISC scores before submitting'));
    }
    ...
    try {
      ...
      const result = await this._userAssessmentService.processAssessment('DISC', {
        scores,
        assessmentResult: this.discFullTextResult$.get(),
      ...
      const errorMessage = error instanceof Error ? error.message : 'Failed to submit DISC assessment';
      this.error$.set(errorMessage);
      return err(error instanceof Error ? error : new Error(errorMessage));
    } finally {
      this.isLoading$.set(false);
